...
const ExperienceList: React.FC = () => {
  return (
    <div className="flex flex-col gap-0 w-full border-t border-black/5">
      {experiences.map((exp) => (
        <Link href={`/experience/${exp.id}`} key={exp.id} className="block w-full">
          <motion.div
          ...
          >
            ...
            <div className="max-w-7xl mx-auto px-4 md:px-20 py-12">
              <div className="flex flex-col md:flex-row md:items-baseline justify-between gap-4 mb-4">
                ...
                <span className="text-gray-500 font-medium text-sm bg-gray-100 px-3 py-1 rounded-full w-fit self-end md:w-auto md:self-auto">{exp.period}</span>
              </div>
